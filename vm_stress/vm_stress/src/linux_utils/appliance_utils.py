...
class ApplianceUtils:
    ...
    def copy_vm_image(self, SrcVmImagepath, DestDatastorepath, VMtemplateName):
        '''
        ...
        handle.send('scp {}@{}:"{}{}" "{}"\n'.format(share_usrName, share_Ip, SrcVmImagepath, VMtemplateName, DestDatastorepath))
        time.sleep(10)
        ...
        if "password:" in output:
            ...
            while retries:
                ...
                if check_prompt:
                    ...
                    time.sleep(30)
        target_file_name = self.appliance_host.execute_command('ls "{}"'.format(DestDatastorepath))
        if VMtemplateName in target_file_name:
            logger.info('file Copied successfully')
        else:
            logger.info('file copying failed')
            return False
        return copy_status
    ...
    def get_vm_images_info(self, vmpath, custom_images=[]):
        ...
        vm_images = self.appliance_host.execute_command('ls {}'.format(vmpath))
    ...
    def OVF_VM_Deploy(self, VMsourceType, DatastoreVolume, NetworkportGroup, VM_name, VMsrcPath, hostid, host_password, host_IP):

        logger.info('vm_image_path is {}'.format(VMsrcPath))
        vmx_file_info = self.get_vm_images_info(VMsrcPath)
        for vmx_file, _ in vmx_file_info:
            if vmx_file.endswith('.vmx'):
    ...
    def check_and_install_ovf_tool(self):
        ...
        if ovf_bin_path in ovf_present_status:
            ...
            ovf_dest_dir = '/root'
            _ = self.copy_vm_image(ovf_src_abs_path, ovf_dest_dir, ovftool_name)
    ...
    def create_share_mount_point(self, mount_ip, mount_dir):
        ...
        check_present_mount = self.appliance_host.execute_command('mount')
