...
esx_6 = "/VMstorage/ESX-VMTemplates/ESX7"
...
class VMUtils:
    ...
    def get_esx_temp_path(self):
        '''
        returns esx vm template path from share location based on ESX version
        '''
        system_version = self.vm_host.execute_command('esxcli system version get')
        logger.info(system_version)
        esx_version = re.findall(r'Version:\s*(.*)', system_version)
        esx_version = esx_version[0]
        
        if (version.parse(esx_version) >= version.parse('6.5')):
            vm_template_path = esx_65
        else:
            vm_template_path = esx_6
        logger.info("Host ESX Version is {} , VM template path is {}".format(esx_version, vm_template_path))
        return vm_template_path
    ...
    def check_vib(self):    
        vib_status =self.vm_host.execute_command("esxcli software vib list | grep qatools")
        vib_status=re.search(r"qatools",str(vib_status))
        if not vib_status:
            logger.info ("vib not installed in the system")
            return True
        else:
            logger.info ("vib installed in the system")
    
    def vib_uninstall(self):
        logger.info ("uninstalling vib tool in esx host")
        command ="esxcli software vib remove --vibname qatools"
        uninstall_vib = self.vm_host.execute_command(command)
        logger.info(uninstall_vib)        
        ...
        if vib_uninstall:
            ...
            self.vm_host.execute_command("reboot")
        else:
            logging.info("vib not  installed susceesfully ")
            exit()
        time.sleep(600)
        
    def file_CopyfromRemote(self,share_username,share_Ip,SrcVmImagepath,DestPath):
        
        self.send('scp {}@{}:"{}{}" "{}"\n'.format(share_username, share_Ip, SrcVmImagepath, DestPath))
        time.sleep(1)
        ...
        if "password:" in output:
            ...
            while retries:
                ...
                output = handle.recv(50000).decode(encoding='utf_8', errors='strict')
                print(output)
                check_prompt = re.findall(r'{}'.format(host_prompt), output)
                if check_prompt:
                    print("host prompt found")
                    copy_status = True
                    break
                else:
                    print('waiting for host prompt')
                    time.sleep(60)
        target_file_name = execute_command(client, 'ls "{}"'.format(DestDatastorepath))
        
    def set_sync_speed(self):
        logger.info ("set syncing speed of esx host")
        set_speedLimit =self.vm_host.execute_command("esxcli storage mpm speedLimit  --min=500000 --max=500000")
        logger.info(set_speedLimit)
        check_mpmSpeedLimit=self.vm_host.execute_command('esxcli storage mpm speedLimit')
        logger.info('MPM speed set to {}'.format(check_mpmSpeedLimit))
        
    def check_sync_status(self):
        while True:
            command="esxcli storage mpm list"
            data=self.vm_host.execute_command(command)
            sync=re.search(r"\w+=\d+\.\d+",str(data))
            finish=re.search(r"\w+=\d+\.\dmin",str(data))
            m =re.findall(r"\[\s\w+_?\w+\s\]",str(data))
            if not  "syncing | resync " in m:
                logger.info ("all disk are synced")
                break
            time.sleep(300)
            logger.info ("Disk are  {0} & {1} please wait after few min will check again".format(sync.group(),finish.group()))
        time.sleep(120)
        logger.info ("Disk are synced now going to appliacne to check duplex state")
